...
runnersLargeSizeShirt=runners.filter(
  function(runner){
    return runner.shirt_size ==='L' || runner.shirt_size ==='XL' ||runner.shirt_size ==='2XL' ;
  }
)
...
// Problem 1 They need to create an eamil list of all donators to remind them about the next event.

let emaillist = [];
emaillist = runners.map(
  function(runner){
    return runner.email;
  }
)
...
// Problem 2  event planner request a list of all the company names to make banners. 

let companyNames = [];
companyNames = runners.map(
  function(runner){
...
// Problem 3  Organization want to email a special thank you note to company that donated more than $100. They need a list of email to send the note.
...
function(runner){
  ...
  if(runner.donation >= 100){
    over100Email.push(runner.email);
  }
